...
loc_638:
		...
		moveq   #7,d7
		clr.w   d6              
						; loop 8 times, for each contextual function pointer available
loc_644:
		...
		movea.l d0,a0
		jsr     (a0)            
						; for each trigger set, execute corresponding contextual function
		movem.l (sp)+,d0-a6
loc_658:
...
loc_906:
		...
		move.l  4(a0),d0
		or.l    (a0),d0         
						; if stack void, do nothing
		beq.w   loc_9F6
		move.b  (a0),d1         
						; contains music volume parameter when command is FC
														; or fade in parameter when 7F < command < F0
		move.b  1(a0),d0        
						; stores first command in d1 and d0 and pushes the rest forward
		move.w  2(a0),(a0)+
loc_91E:
...
loc_994:
		...
		movem.l d0,-(sp)
		andi.b  #$7F,d0 
						; a music/sfx index mask that must be changed to allow indexes above $80
		...
		bne.s   loc_9AA
		bra.w   loc_9F6         
						; if same to last played music, let current music play
														; (might play nothing if last played music has ended !)
loc_9AA:
		
		bclr    #7,d0           
						; if command > 7F, then send Fade In parameter
		beq.s   loc_9B8         
						; branch if bit was 1 before bclr
...
loc_9DC:
		...
		move.b  -2(a0),-(a0)    
						; add music index to music stack
		dbf     d7,loc_9DC
		move.b  d0,-(a0)
		movem.l (sp)+,d7-a0
...
FadingData:
		incbin "data/graphics/tech/fadingdata.bin"
						; 80 : end
														; 8x : go x backward
...
loc_B9A:
		...
		btst    #1,d0           
						; control if DMA in progress
		bne.s   loc_B9A         
						; loop if DMA in progress
		movem.w (sp)+,d0
		rts
...
DisableInterrupts:
		...
		move    #$2700,sr       
						; set interrupt mask to level 7 : no more HInt/VInt !
		rts
...
DmaSpriteTable:
		...
		lea     (VDP_Control).l,a6
		move.w  #$8134,(a6)     
						; disable display, enable Vint, enable DMA
		move.l  #$94019300,(a6) 
						; DMA Length : $100
		move.l  #$96EE9540,(a6) 
						; from RAM 0xFFEE40
...
DmaVramFill:
		...
		ori.b   #$10,d3         
						; make sure it concerns a 16+ vdp reg
		move.w  d3,(VDP_Control).l
						; send command again
		move.w  #$8F01,(VDP_Control).l
		...
		andi.w  #3,d0
		ori.w   #$80,d0 
						; errr .. CD5 set to 1 ? doesn't correspond to any access mode
		move.w  d0,(VDP_Control).l
