...
; ASM FILE code\common\tech\interrupts\vintengine-standard.asm :


; =============== S U B R O U T I N E =======================================

; Vertical Interrupt - Triggered at each frame display
...
VInt:           movem.l d0-a6,-(sp)
                ...
                bsr.w   WaitDmaEnd
                andi.b  #$BF,(VDP_REG01_VALUE).l                ; disable display
...
@Loop:          move.w  (VDP_Data).l,(a0)+
                dbf     d7,@Loop
                
@loc_1:         bsr.w   ApplyFadingEffectAndZ80BusUpdates
...
@Continue:      move.b  d0,((SECONDS_COUNTER_FRAMES-$1000000)).w
...
@SendCommand:   move.w  (a0)+,(a6)      ; Send all queued DMA commands
                move.l  (a0)+,(a6)
                move.l  (a0)+,(a6)
                move.l  (a0)+,(a6)
