
; ASM FILE code\gameflow\battle\battlescenes\animation\update\neptun.asm :
; 0x1DFD4..0x1E134 : Battlescene engine

; =============== S U B R O U T I N E =======================================


spellanimationUpdate_Neptun:
                
                lea     ((SPELLANIMATION_PROPERTIES-$1000000)).w,a5
                tst.w   (a5)
                beq.w   loc_1E010
                addq.w  #1,(a5)
                cmpi.w  #40,(a5) 
                bne.s   loc_1DFEA
                clr.b   ((byte_FFB588-$1000000)).w
loc_1DFEA:
                
                subq.b  #1,4(a5)
                bne.w   loc_1E010
                move.b  #3,4(a5)
                lea     table_Rain_Xoffsets(pc), a0
                lea     ((SPRITE_01-$1000000)).w,a4
                moveq   #7,d0
                bsr.w   MakeRainStreak
                lea     ((SPRITE_38-$1000000)).w,a4
                moveq   #10,d0
                bsr.w   MakeRainStreak
loc_1E010:
                
                lea     12(a5),a5
                lea     ((SPRITE_60-$1000000)).w,a4
                moveq   #60,d0 
                moveq   #2,d1
loc_1E01C:
                
                movem.w d0-d1,-(sp)
                tst.w   (a5)
                beq.w   loc_1E0C0
                addq.w  #1,(a5)
                move.w  2(a5),d2
                bne.w   loc_1E05E
                subq.w  #1,4(a5)
                bne.w   loc_1E0C0
                lea     graphic_NeptuneSizzle(pc), a0
                bsr.w   ConstructSimpleGraphic
                move.w  #256,d6
                jsr     (GenerateRandomNumber).w
                add.w   d7,VDPSPRITE_OFFSET_X(a4)
                addq.w  #1,2(a5)
                move.w  #5,4(a5)
                sndCom  SFX_WING_FLAP
                bra.w   loc_1E0C0
loc_1E05E:
                
                cmpi.w  #3,d2
                bcc.w   loc_1E090
                subq.w  #1,4(a5)
                bne.w   loc_1E0C0
                addi.w  #4,VDPSPRITE_OFFSET_TILE(a4)  ; progress sizzle graphic
                addq.w  #1,2(a5)
                cmpi.w  #2,d2
                bcc.s   loc_1E086
                move.w  #5,4(a5)
                bra.s   loc_1E08C
loc_1E086:
                
                move.w  #12,4(a5)
loc_1E08C:
                
                bra.w   loc_1E0C0
loc_1E090:
                
                subq.w  #1,4(a5)
                beq.w   loc_1E0AE
                btst    #0,5(a5)
                bne.s   loc_1E0A6
                move.w  #256,(a4)
                bra.s   loc_1E0AA
loc_1E0A6:
                
                move.w  #1,(a4)
loc_1E0AA:
                
                bra.w   loc_1E0C0
loc_1E0AE:
                
                clr.w   2(a5)
                moveq   #8,d6
                jsr     (GenerateRandomNumber).w
                addi.w  #4,d7
                move.w  d7,4(a5)
loc_1E0C0:
                
                movem.w (sp)+,d0-d1
                addq.w  #1,d0
                lea     12(a5),a5
                addq.w  #VDP_SPRITE_ENTRY_SIZE,a4
                dbf     d1,loc_1E01C
                
                tst.w   ((byte_FFB404-$1000000)).w
                beq.w   ReinitializeSceneAfterSpell
                rts

    ; End of function spellanimationUpdate_Neptun


; =============== S U B R O U T I N E =======================================


MakeRainStreak:
                moveq   #112,d6 
                jsr     (GenerateRandomNumber).w
				
                addi.w  #152,d7 
                move.w  d7,(a4)+
                move.w  #VDPSPELLPROP_V4|VDPSPELLPROP_H2,(a4)+
                move.w  #VDPTILE_SPELLTILE19|VDPTILE_PALETTE3|VDPTILE_PRIORITY,(a4)+
                move.w  (a0),(a4)+
				
                addi.w  #32,d7 
                move.w  d7,(a4)+
                move.w  #VDPSPELLPROP_V2|VDPSPELLPROP_H2,(a4)+
                move.w  #VDPTILE_SPELLTILE27|VDPTILE_PALETTE3|VDPTILE_PRIORITY,(a4)+
                move.w  (a0)+,(a4)+
                dbf     d0,MakeRainStreak
                rts

    ; End of function MakeRainStreak

graphic_NeptuneSizzle:
                vdpSpell 128, 256, SPELLTILE31, V2|H2|VALUE2
                
table_Rain_Xoffsets:
                dc.w 136
                dc.w 149
                dc.w 162
                dc.w 174
                dc.w 189
                dc.w 201
                dc.w 214
                dc.w 227
                dc.w 241
                dc.w 253
                dc.w 267
                dc.w 279
                dc.w 293
                dc.w 306
                dc.w 319
                dc.w 331
                dc.w 346
                dc.w 358
                dc.w 376
