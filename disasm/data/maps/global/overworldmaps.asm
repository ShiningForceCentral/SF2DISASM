
; ASM FILE data\maps\global\overworldmaps.asm :
; 0x4428A..0x44298 : Overworld maps
tbl_OverworldMaps:
                dc.b MAP_OVERWORLD_GRANS_GRANSEAL
                dc.b MAP_OVERWORLD_DEVILS_TAIL
                dc.b MAP_OVERWORLD_AROUND_ELVEN_VILLAGE
                dc.b MAP_OVERWORLD_PATH_TO_RIBBLE
                dc.b MAP_OVERWORLD_SOUTHEAST_PARMECIA
                dc.b MAP_OVERWORLD_AROUND_PACALON
                dc.b MAP_OVERWORLD_NORTH_SOUTH_PARMECIA_JUNCTION
                dc.b MAP_OVERWORLD_NEW_GRANSEAL_SHORE
                dc.b MAP_OVERWORLD_MOUN_AND_MITULA
                dc.b MAP_OVERWORLD_GRANS_NORTH_SHORE
                dc.b MAP_OVERWORLD_GRANS_RETURN_PATH
                dc.b MAP_OVERWORLD_GRANS_AROUND_DWARF_VILLAGE
                dc.b MAP_OVERWORLD_PACALON_2
                
                tableEnd.b
